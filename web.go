...
func (tm *TaskManager) WebHandler(baseURL string) *http.ServeMux {
	...
	// Normalize baseURL - ensure it starts with / and doesn't end with /
	...
	// Register routes
	if baseURL == "" {
		mux.HandleFunc("/", tm.handleIndex(baseURL))
		mux.HandleFunc("/action", tm.handleTaskAction(baseURL))
		mux.HandleFunc("/stats", tm.handleStats(baseURL))
	} else {
		mux.HandleFunc(baseURL+"/", tm.handleIndex(baseURL))
		mux.HandleFunc(baseURL+"/action", tm.handleTaskAction(baseURL))
...
func (tm *TaskManager) handleIndex(baseURL string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		msgType := r.URL.Query().Get("status")

		// Get refresh interval (default 10 seconds, 0 means no refresh)
		refreshStr := r.URL.Query().Get("refresh")
		refresh := 10 // default 10 seconds
		if refreshStr != "" {
			if val, err := time.ParseDuration(refreshStr + "s"); err == nil {
				refresh = int(val.Seconds())
			}
		}

		tm.renderIndexWithMessage(w, baseURL, msg, msgType, refresh)
	}
}
...
func (tm *TaskManager) handleTaskAction(baseURL string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Preserve refresh parameter if present
		...
		if refreshParam != "" {
			redirectURL += "?status=" + messageType + "&msg=" + message + "&refresh=" + refreshParam
		} else {
			redirectURL += "?status=" + messageType + "&msg=" + message
		}

		// Use 303 See Other to ensure browser does GET after POST
		// This prevents "confirm form resubmission" on refresh
...
func (tm *TaskManager) renderIndexWithMessage(w http.ResponseWriter, baseURL, message, messageType string, refresh int) {
	tasks := tm.ListTasks()
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="nav">
                <a href="{{.BaseURL}}{{if .BaseURL}}/{{end}}{{if gt .Refresh 0}}?refresh={{.Refresh}}{{end}}">📋 Tasks</a>
                <a href="{{.BaseURL}}/stats">📊 Statistics</a>
            </div>
            <div class="refresh-control">
                <label for="refresh-select">🔄 Auto-refresh:</label>
                <select id="refresh-select" onchange="window.location.href='{{.BaseURL}}{{if .BaseURL}}/{{end}}?refresh='+this.value">
                    <option value="0" {{if eq .Refresh 0}}selected{{end}}>Disabled</option>
                    <option value="5" {{if eq .Refresh 5}}selected{{end}}>5 seconds</option>
                    <option value="10" {{if eq .Refresh 10}}selected{{end}}>10 seconds</option>
                    <option value="15" {{if eq .Refresh 15}}selected{{end}}>15 seconds</option>
                    <option value="30" {{if eq .Refresh 30}}selected{{end}}>30 seconds</option>
                    <option value="60" {{if eq .Refresh 60}}selected{{end}}>1 minute</option>
                </select>
                {{if gt .Refresh 0}}<span class="refresh-status">✓ Refreshing every {{.Refresh}}s</span>{{end}}
            </div>
        </div>
        ...
        <div class="{{if eq .MessageType "error"}}error-message{{else}}success-message{{end}}">
            ...
            <a href="{{.BaseURL}}{{if .BaseURL}}/{{end}}{{if gt .Refresh 0}}?refresh={{.Refresh}}{{end}}" class="message-close" title="Close">×</a>
        </div>
        {{end}}
            ...
            <div class="task-grid">
                ...
                <div class="task-card">
                    ...
                    <div class="task-actions">
                        {{if .Enabled}}
                        <form method="POST" action="{{$.BaseURL}}/action{{if gt $.Refresh 0}}?refresh={{$.Refresh}}{{end}}" style="display: inline;">
                        ...
                        <form method="POST" action="{{$.BaseURL}}/action{{if gt $.Refresh 0}}?refresh={{$.Refresh}}{{end}}" style="display: inline;" onsubmit="return confirm('Are you sure you want to remove this task?');">
                            <input type="hidden" name="name" value="{{.Name}}">
